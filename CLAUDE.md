...
- Brand tokens — **type:** **Newsreader** (variable serif) for **headlines** (`.display` H1 + `h2`, plus the calculator verdict title; opsz baked to display, weight 400–700; self-hosted single WOFF2); **Atkinson Hyperlegible** (self-hosted, **static 400/700**, Braille Institute, OFL) for **body reading text only** — prose: `.lead`, section intros, step/card/FAQ/About/how/calculator copy (in-prose bold = 700; chosen **2026-06-19** for max legibility, older audience). **Source Sans 3** (variable humanist sans, self-hosted single WOFF2, weight 300–800) for the **UI chrome** — eyebrows, buttons, nav, tags, `h3`/`h4`, SVG labels (600 nav links, 800 nav wordmark). Atkinson is **scoped to prose** because it lacks the 500/600/800 the chrome weight-ladder needs (a wholesale swap would need "Atkinson Hyperlegible Next", the variable cut); Source Sans was chosen 2026-06-16 over Poppins for cohesion + legibility. Poppins is **retired** from the live pages — kept only because `assets/og/` generators reference its WOFF2. Body type scale bumped for an older audience. **colour:** navy `#2F3B49` · teal `#3E9DAE` · **accent-deep `#2E7785`** (web-only token: brand teal is 3.0:1 on white, so small text/CTAs use accent-deep at ≥4.5:1; bright teal is for borders, bullets, large accents, and accents on navy) · accent-soft `#EAF4F6` · **paper-cream `#F7F2E7`** (the calculator "worksheet" surface).
