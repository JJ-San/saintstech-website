...
- Brand tokens — **type:** **Newsreader** (variable serif) for **headlines only** (`.display` H1 + `h2`; opsz baked to display, weight 400–700; self-hosted single WOFF2); **Poppins** for everything else — body, eyebrows, buttons, nav, `h3`/`h4` (Light 300 / Regular 400 / Medium 500 / SemiBold 600 / Bold 700 / ExtraBold 800 — 600 is nav links, 800 is the nav wordmark). Body type scale was bumped for an older audience (2026-06-16). **colour:** navy `#2F3B49` · teal `#3E9DAE` · **accent-deep `#2E7785`** (web-only token: brand teal is 3.0:1 on white, so small text/CTAs use accent-deep at ≥4.5:1; bright teal is for borders, bullets, large accents, and accents on navy) · accent-soft `#EAF4F6`.
